...
void FullPairwiseAlign::pairwiseAlign(Alignment *alignPtr, DistMatrix *distMat, int iStart, int iEnd, int jStart, int jEnd)
{
	...
	//load observed and expected frequencies matrices
	cout << "Loading Observed and Expected Frequencies...\n";
	...
	cout << "Observed and Expected Matrices loaded.\n";
	// Get the names of the sequences from the Fasta file
	// and generate the .seq files
	cout << "Generating .seq files...\n";
	vector<string> seqNames = readFasta(userParameters->getSeqName());
	cout << "Done.\n";
	int seqNum1 = 0;
	int seqNum2 = 1;
	cout << "Pairwise aligning the sequences...\n";
        for (si = utilityObject->MAX(0, iStart); si < _numSeqs && si < iEnd; si++)
        {
          //read the SEQ file for seq1
	  string seque1 = readSeq(seqNames[si]+".seq");
	  //generate GCA file for this sequence
	  int nres1 = seq2gca(seqNames[si]+".seq");
	  //get the gamma values for this sequence
	  vector<vector<double> > gamma1 = readGamma(seqNames[si]+".gca", nres1);
	  ...
	  n = alignPtr->getSeqLength(si + 1);
            ...
            {
              //read the SEQ file for seq2
	      string seque2 = readSeq(seqNames[sj]+".seq");
	      //generate GCA file for this sequence
	      int nres2 = seq2gca(seqNames[sj]+".seq");
	      //get the gamma values for this sequence
	      vector<vector<double> > gamma2 = readGamma(seqNames[sj]+".gca", nres2);
	      
	      //make the alignment matrix for these two sequences
	      alignM = makeMij(seque1, seque2, gamma1, gamma2, obsMat, expMat);
	...
	//cleanup the generated GCA and SEQ files
	cleanupFiles(seqNames);
...
inline int FullPairwiseAlign::calcScore(int iat, int jat, int v1, int v2)
{
  ...
  //---Haidar Khan
  //REGION OF MAJOR CHANGE
  return alignM[(v1+iat)-1][(v2+jat)-1];
